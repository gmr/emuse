...
async def lifespan() -> abc.AsyncIterator[psycopg_pool.AsyncConnectionPool]:
    ...
    async with psycopg_pool.AsyncConnectionPool(
            ...
            check=psycopg_pool.AsyncConnectionPool.check_connection) as pool:
        yield pool
...
async def connection(request: fastapi.Request) \
...
InjectConnection = typing.Annotated[ConnectionType,
                                    fastapi.Depends(connection)]


@contextlib.asynccontextmanager
async def cursor(conn: ConnectionType,
                 row_factory_class: ModelType | None = None) \
